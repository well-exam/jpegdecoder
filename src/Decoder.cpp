...
namespace kpeg {
    ...
    Decoder::ResultCode Decoder::parseSegmentInfo(const UInt8 byte) {
        ...
        switch (byte) {
            case JFIF_SOI :
                logFile << "Found segment, Start of Image (FFD8)" << std::endl;
                return SUCCESS;
            case JFIF_APP0:
                logFile << "Found segment, JPEG/JFIF Image Marker segment (APP0)" << std::endl;
                parseAPP0Segment();
                return SUCCESS;
            case JFIF_COM  :
                logFile << "Found segment, Comment(FFFE)" << std::endl;
                parseCOMSegment();
                return SUCCESS;
            case JFIF_DQT  :
                logFile << "Found segment, Define Quantization Table (FFDB)" << std::endl;
                parseDQTSegment();
                return SUCCESS;
            case JFIF_SOF0 :
                logFile << "Found segment, Start of Frame 0: Baseline DCT (FFC0)" << std::endl;
                return parseSOF0Segment();
            case JFIF_SOF1 :
                logFile << "Found segment, Start of Frame 1: Extended Sequential DCT (FFC1), Not supported"
                        << std::endl;
                return TERMINATE;
            case JFIF_SOF2 :
                logFile << "Found segment, Start of Frame 2: Progressive DCT (FFC2), Not supported" << std::endl;
                return TERMINATE;
            case JFIF_SOF3 :
                logFile << "Found segment, Start of Frame 3: Lossless Sequential (FFC3), Not supported" << std::endl;
                return TERMINATE;
            case JFIF_SOF5 :
                logFile << "Found segment, Start of Frame 5: Differential Sequential DCT (FFC5), Not supported"
                        << std::endl;
                return TERMINATE;
            case JFIF_SOF6 :
                logFile << "Found segment, Start of Frame 6: Differential Progressive DCT (FFC6), Not supported"
                        << std::endl;
                return TERMINATE;
            case JFIF_SOF7 :
                logFile << "Found segment, Start of Frame 7: Differential lossless (Sequential) (FFC7), Not supported"
                        << std::endl;
                return TERMINATE;
            case JFIF_SOF9 :
                logFile
                        << "Found segment, Start of Frame 9: Extended Sequential DCT, Arithmetic Coding (FFC9), Not supported"
                        << std::endl;
                return TERMINATE;
            case JFIF_SOF10:
                logFile << "Found segment, Start of Frame 10: Progressive DCT, Arithmetic Coding (FFCA), Not supported"
                        << std::endl;
                return TERMINATE;
            case JFIF_SOF11:
                logFile
                        << "Found segment, Start of Frame 11: Lossless (Sequential), Arithmetic Coding (FFCB), Not supported"
                        << std::endl;
                return TERMINATE;
            case JFIF_SOF13:
                logFile
                        << "Found segment, Start of Frame 13: Differential Sequential DCT, Arithmetic Coding (FFCD), Not supported"
                        << std::endl;
                return TERMINATE;
            case JFIF_SOF14:
                logFile
                        << "Found segment, Start of Frame 14: Differential Progressive DCT, Arithmetic Coding (FFCE), Not supported"
                        << std::endl;
                return TERMINATE;
            case JFIF_SOF15:
                logFile
                        << "Found segment, Start of Frame 15: Differential Lossless (Sequential), Arithmetic Coding (FFCF), Not supported"
                        << std::endl;
                return TERMINATE;
            case JFIF_DHT  :
                logFile << "Found segment, Define Huffman Table (FFC4)" << std::endl;
                parseDHTSegment();
                return SUCCESS;
            case JFIF_SOS  :
                logFile << "Found segment, Start of Scan (FFDA)" << std::endl;
                parseSOSSegment();
                return SUCCESS;
        }
    ...
    Decoder::ResultCode Decoder::decodeImageFile() {
        if (!m_imageFile.is_open() || !m_imageFile.good()) {
            logFile << "Unable to scan image file: \'" + m_filename + "\'" << std::endl;
            return ERROR;
        }
        ...
        while (m_imageFile >> std::noskipws >> byte) {
            if (byte == JFIF_BYTE_FF) {
                ...
                if (code == SUCCESS)
                    ...
                    status = TERMINATE;
                    break;
                } else if (code == DECODE_INCOMPLETE) {
                    status = DECODE_INCOMPLETE;
                    break;
                }
            } else {
        ...
        } else if (status == DECODE_INCOMPLETE) {
    ...
    void Decoder::parseAPP0Segment() {
        ...
        logFile << "JFIF version: " << (int) majVersionByte << "." << (int) (minVersionByte >> 4)
                << (int) (minVersionByte & 0x0F) << std::endl;
    ...
    void Decoder::parseDQTSegment() {
        ...
        for (int qt = 0; qt < int(lenByte) / 65; ++qt) {
            ...
            logFile << "Quantization Table #" << QTtable << " precision: " << (precision == 0 ? "8-bit" : "16-bit")
                    << std::endl;
    ...
    Decoder::ResultCode Decoder::parseSOF0Segment() {
        ...
        for (auto i = 0; i < 3; ++i) {
            ...
            logFile << "Sampling Factor, Horizontal: " << int(sampFactor >> 4) << ", Vertical: "
                    << int(sampFactor & 0x0F) << std::endl;
        ...
        while (m_imageFile.tellg() < segmentEnd) {
            ...
            for (auto i = 0; i < 16; ++i) {
                ...
                for (auto &&symbol : m_huffmanTable[HTType][HTNumber][i].second) {
                    std::stringstream ss;
                    ss << "0x" << std::hex << std::setfill('0') << std::setw(2) << std::setprecision(16)
                       << (int) symbol;
                    codeStr += ss.str() + " ";
                    totalCodes++;
                }
            ...
            logFile << "Total Huffman codes for Huffman table(Type:" << HTType << ",#:" << HTNumber << "):"
                    << totalCodes << std::endl;
    ...
    void Decoder::parseSOSSegment() {
        ...
        if (compCount < 1 || compCount > 4) {
            logFile << "Invalid component count in image scan: " << (int) compCount
                    << ", terminating decoding process..." << std::endl;
            return;
        }

        logFile << "NUmber of component in scan data: " << (int) compCount << std::endl;
        ...
        for (auto i = 0; i < compCount; ++i) {
            ...
            logFile << "Component ID: " << (int) cID << ", DC Table #: " << (int) DCTableNum << ", AC Table #: "
                    << (int) ACTableNum << std::endl;
        }
    ...
    void Decoder::parseCOMSegment() {
        ...
        for (auto i = 0; i < lenByte - 2; ++i) {
            ...
            if (byte == JFIF_BYTE_FF) {
                logFile << "Unexpected start of marker at offset: " << curPos + i << std::endl;
    ...
    void Decoder::decodeScanData() {
        ...
        for (auto i = 0; i < MCUCount; ++i) {
            ...
            //
            // NOTE:
            // Since a significant portion of a RLE for a
            // component contains a trail of 0s, AC coefficients
            // are decided till, either an EOB (End of block) is
