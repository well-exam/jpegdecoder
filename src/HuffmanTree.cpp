// Implementation of the Huffman tree abstraction
...
namespace kpeg {
    void insertLeft(NodePtr node, UInt16 value) {
        if (node == nullptr)
            return;

        if (node->lChild != nullptr) {
            logFile << "Given node already a left child, skipping insertion" << std::endl;
            return;
        }
    ...
    void insertRight(NodePtr node, UInt16 value) {
        if (node == nullptr)
            return;

        if (node->rChild != nullptr) {
            logFile << "Given node already a right child, skipping insertion" << std::endl;
            return;
        }
    ...
    NodePtr getRightLevelNode(NodePtr node) {
        ...
        // Else node is the right child if its parent, then traverse
    ...
    void inOrder(NodePtr node) {
        ...
        if (node->code != "" && node->leaf)
            logFile << "Symbol: 0x" << std::hex << std::setfill('0') << std::setw(2) << std::setprecision(16)
                    << node->value << ", Code: " << node->code << std::endl;
    ...
    void HuffmanTree::constructHuffmanTree(const HuffmanTable &htable) {
        ...
        for (auto i = 1; i <= 16; ++i) {
            ...
            } else { // Else assign codes to nodes starting from leftmost leaf in the tree.
